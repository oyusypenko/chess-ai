...
| Playwright browser suite   | done  | 206 tests across 360 px and 1280 px; 360 px is the baseline project, not an extra (D-08)                                                                                                                 |
| Email + password auth      | done  | PBKDF2 600k (measured, D-10), DB-backed throttling, identical failure message + fake verify against enumeration, session list/revoke. Found a real `Secure`-cookie bug: worked on click, failed on fetch |
| Pre-commit hook            | done  | `.githooks/pre-commit` via `prepare`: typecheck, lint, format, unit, hard rules, build, FR-7 smoke, full browser suite — ~50 s. `SKIP_E2E=1` for the fast path                                           |
...
| 8   | **No password reset.** Email+password sign-in shipped without it, because there is no email sender. Anyone who forgets a password is locked out with no self-service path — a support burden, not a gap we can defer quietly. | US-A2, D-10    | needs an email provider decision                      |
| 9   | **No email verification.** `email_verified_at` exists and is always NULL. Addresses are unproven, so anyone can register with someone else's, and we cannot safely use the address for recovery.                              | US-A2, D-10    | same blocker as #8                                    |
