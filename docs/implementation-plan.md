...
a **clean-room re-implementation** — take the *methodology* (win-probability-delta thresholds,
...
Vercel supports COOP/COEP via `next.config` and is the fastest path for P0. NFR-PR1 prefers
EU-region hosting — set Vercel function region to EU (fra1) for P0; revisit (Hetzner/Fly + EU
Postgres) at P1 when the DB lands. Rate limiting and quota state need server-side storage even in
P0 → Upstash Redis (EU region).

---

## 2. Stack decision (proposed)

> PRD §8 marks the architecture informative. This is the concrete proposal — flag disagreement
> before M1 starts.
...
- **Server state:** Upstash Redis (EU) — IP rate limits (US-A1), later free-tier quotas (US-F1).
  Postgres (Neon/Supabase EU) arrives in P1 with accounts.
...
deploy pipeline (Vercel, EU region). *Unblocks the format hook from Phase 0.*
...
1. Confirm stack proposal (§2) — esp. single Next.js app + Vercel-EU for P0.
...
- [COOP/COEP guide (web.dev)](https://web.dev/articles/coop-coep) · [Vercel SharedArrayBuffer KB](https://vercel.com/kb/guide/fix-shared-array-buffer-not-defined-nextjs-react) · [MDN COEP](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Cross-Origin-Embedder-Policy)
